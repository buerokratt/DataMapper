export const extractMessageInfo = (message, previousMessage, csaTitleVisible, csaNameVisible) => {
  ...
  const content = message.content ? tryUnesacpe(message.content) : message.content;
...
const extractContent = (message, previousMessage, content) => {
  if (previousMessage?.buttons && previousMessage?.authorRole !== 'end-user' && message.authorRole === 'end-user') {
    const selectedButton = JSON.parse(previousMessage.buttons).find((button) => button.payload === message.content);
